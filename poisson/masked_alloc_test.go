...
// PCG solve. After warm-up every Solve draws its work vectors from the resident
// pool and calls the inner spectral solve once per iteration, so the honest
// bound is "no more than the inner solves cost, plus a tiny constant" — a growing
// wrapper (e.g. a per-call scratch allocation) would blow past it. Gated on !race
// because the race detector inflates allocation counts.
func TestMasked_AllocFree(t *testing.T) {
	const nx, ny = 24, 20
	size := nx * ny
	n := []int{nx, ny}
	h := []float64{0.1, 0.12}
	...
	plan, err := poisson.NewMaskedPlan(2, n, h, bcs, mask, poisson.WithMaskParallelism(1))
	if err != nil {
